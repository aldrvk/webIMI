...
SELECT
    ...
    ) AS total_event_selesai,
    
    (
        SELECT COUNT(id) 
        FROM kis_applications 
        WHERE status = 'Pending'
    ) AS total_kis_pending
$$

CREATE VIEW `View_Finished_Events` AS
SELECT 
    e.*,
    c.nama_klub AS proposing_club_name
FROM events AS e
...
SELECT 
    ...
    u.name AS pembalap_name,
    u.email AS pembalap_email,
    kc.nama_kategori AS category_name,
    kc.kode_kategori AS category_code
...
WHERE
    er.result_position IS NOT NULL
    OR er.points_earned > 0
$$
